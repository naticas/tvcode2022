...
function findClosest(from, target, direction) {
  ...
  try {
    ...
    $.each(target, function (i, el) {
      ...
      var dist = Math.sqrt(
        Math.pow(offset.left + cx2 - from_offset.left - cx, 2) +
          Math.pow(offset.top + cy2 - from_offset.top - cy, 2)
      );
      if (dist < shortest) {
        shortest = dist;
        index = el;
...
function setLoading(load) {
  if (document.getElementById('loading')) {
    document.getElementById('loading').removeClass(load ? 'hide' : 'show');
    document.getElementById('loading').addClass(load ? 'show' : 'hide');
  }
  loading = load;
}
...
function getElementByAttributeValue(elementArray, attributeName, attributeValue) {
  try {
    for (var i = 0; i < elementArray.length; i++)
      if (elementArray[i].getAttribute(attributeName) === attributeValue) return elementArray[i];
    return null;
  } catch (e) {
    console.log(e);
  }
...
function parseServiceList(data, dvbChannels, maxLcn) {
  //debugger;
  var list = [];
  //XML parser DOMParser XML-->object
  var parser = new DOMParser();
  //parseFromString : data, type method
  var doc = parser.parseFromString(data, 'text/xml');
  //service + content guide presentation
  ...
  if (contentGuides.length > 0) {
    contentGuideURI = contentGuides[0]
  ...
  for (var i = 0; i < services.length; i++) {
    ...
    for (var j = 0; j < relatedMaterial.length; j++) {
      var howRelated = relatedMaterial[j]
        .getElementsByTagNameNS('urn:tva:metadata:2019', 'HowRelated')[0]
        .getAttribute('href');
      if (howRelated == 'urn:dvb:metadata:cs:HowRelatedCS:2019:1001.2') {
        chan.image = relatedMaterial[j]
          .getElementsByTagNameNS('urn:tva:metadata:2019', 'MediaLocator')[0]
    ...
    for (var j = 0; j < serviceInstances.length; j++) {
      var sourceType =
        serviceInstances[j].getElementsByTagName('SourceType')[0].childNodes[0].nodeValue;
      ...
      if (
        dvbChannels &&
        (sourceType == 'urn:dvb:metadata:source:dvb-t' ||
        ...
        for (var k = 0; k < dvbChannels.length; k++) {
          ...
          if (
            dvbChannel.sid == triplet.getAttribute('serviceId') &&
      ...
      if (sourceType == 'urn:dvb:metadata:source:my-channels') {
        sourceTypes.push('DVB-DASH');
        chan.dashUrl = serviceInstances[j].getElementsByTagName('URI')[0].childNodes[0].nodeValue;
      }
    }
    if (!sourceTypes.length) continue;
  ...
  for (let i = 0; i < list.length; i++) if (!list[i].lcn) list[i].lcn = ++maxLcn;
  return { list, maxLcn };
}
...
window.addEventListener('keydown', (e) => {
  if (e.keyCode === 38 || e.keyCode === 40) {
    let channel = channels.find((channel) => channel.selected);
    console.log('playcheck');

    if (channel) {
      if (e.keyCode === 38) {
        console.log('↑');
        let currentIndex = channels.findIndex((data) => data === channel);
        ...
        //   channels = channels.map(channel => ({ ...channel, selected: false }));
        //   channels[nextIndex].selected = true;
        //   player.attachSource(channels[nextIndex].dashUrl);
        console.log(channels[nextIndex].id);
        channelSelected(channels[nextIndex].id);
        ...
        //   channels = channels.map(channel => ({...channel, selected: false }));
        //   channels[prevIndex].selected = true;
        //   player.attachSource(channels[prevIndex].dashUrl);
        console.log(channels[prevIndex].id);
        channelSelected(channels[prevIndex].id);
      }
    } else {
      // channels = channels.map(channel => ({...channel, selected: false }));
      // channels[0].selected = true;
      // player.attachSource(channels[0].dashUrl);
